...
if FlowType == 1 % Only do this for venturis
    P_up_check = P_d/0.7999; % Calculate minimum upstream pressure to remain choked
    Min_possible_mdot = solveGas_mdot(P_up_check,T_up,P_d,CdA,fluid,FlowType); % Calculate lowest mass flow the choked venturi can provide
    if Min_possible_mdot > mdot_lbm
        Max_possible_CdA = solveGas_CdA(P_up_check,T_up,P_d,mdot_lbm,fluid,FlowType); % Obtain max possible flow area to provide critical metering of the mass flow requested
...
while abs(mdot_dif) > mdot_tol
    ...
    mdot_guess = solveGas_mdot(P_up_guess,T_up,P_d,CdA,fluid,FlowType); %[lbm/s]
